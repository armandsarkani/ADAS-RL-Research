...
def trajectory(next, lane_id_init, counter):
    if(next is None):
        return
    wp = random.choice(next.next(10.0))
    if(wp is None):
        return
    counter += 1
    if(wp.lane_id != lane_id_init and ((wp.lane_id - lane_id_init) == 1 or (wp.lane_id - lane_id_init) == -1)):
        print(counter, " waypoints needed to reach next lane (lane id = ", wp.lane_id, ")")
        return
    trajectory(wp, lane_id_init, counter)
...
try:
   ...
   # waypoint = carla_map.get_waypoint(location, project_to_road=True)
    ...
    while True:
        ...
        print("Current lane id: ", car.lane_id)
        trajectory(random.choice(car.next(10.0)), car.lane_id, 0)
